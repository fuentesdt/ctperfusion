...
-x:  this mask -- defined in the 'fixed' image space defines the region of interest over which the registration is computed ==> above 0.1 means inside mask ==> continuous values in range [0.1,1.0] effect optimization like a probability. ==> values > 1 are treated as = 1.0
...
while getopts "d:f:r:i:h:o:m:n:l:q:s:t:x:" OPT
do
    case $OPT in
    h) #help
        echo "$Help"
...
if  [ ${#MASKIMAGE} -gt 3 ]
then
cp ${MASKIMAGE} ${tmpdir}/mask`basename ${MASKIMAGE}`
fi
cd ${tmpdir}/
...
if  [ ${#MASKIMAGE} -gt 3 ]
then
MASKIMAGE=${tmpdir}/mask`basename ${MASKIMAGE}`
fi
...
echo "MASKIMAGE=$MASKIMAGE" >> ${MOVINGBASE}.cfg
...
if  [ ${#MASKIMAGE} -gt 3 ]
then
exe="${ANTSPATH}ANTS $DIM -m  ${METRIC}${FIXED},${MOVING},${METRICPARAMS} -t $TRANSFORMATION -r $REGULARIZATION -o ${OUTPUTNAME} -i $MAXITERATIONS --use-Histogram-Matching  --number-of-affine-iterations $AFFINEITERATIONS --MI-option 32x16000 --mask-image ${MASKIMAGE}"
else
