...
Processed/%/dynamicantsreg.nhdr: 
	...
	cd $(@D); /opt/apps/ANTS/dev/install/bin/antsMotionCorr -v 1 -d 3 -o [motcorrgcgd,motcorrgcgd.nii.gz,motcorrgcgdavg.nii.gz] -m gc[ dynamic.0000.nii.gz , dynamicc4d.nii.gz , 1 , 1 , Random, 0.05  ] -t Affine[ 0.01 ] -u 0 -i 20 -e 1 -s 0 -f 1  -m CC[  dynamic.0000.nii.gz , dynamicc4d.nii.gz , 1 , 4 ] -t GaussianDisplacementField[0.15,3,0.5] -u 0 -i 20 -e 1 -s 0 -f 1 > origantsmotioncorr.log 2>&1;
...
Processed/0004/setup:
	...
	c3d -verbose -mcs  '/mnt/FUS4/data2/ethompson/CT_Perfusion/ZPAF19L017_Processed/10.31.2019_Processed/DynMulti4D  1.5  B20f 34 - as a 34 frames MultiVolume by ImagePositionPatientAcquisitionTime.nhdr' -foreach -region 140x130x50vox 256x256x48 -endfor -oo $(@D)/dynamic.%04d.nii.gz ; c4d -verbose -mcs $(@D)/dynamic.????.nii.gz -tile w  -o   Processed/0004/dynamicc4d.nii.gz
