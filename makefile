...
Processed/%/dynamicantsreg.nhdr: 
	...
	cd $(@D); /opt/apps/ANTS/dev/install/bin/antsMotionCorr -v 1 -d 3 --use-histogram-matching 1 -o [motcorr,motcorr.nii.gz] --transform Rigid[ 0.1 ] --metric MI[ dynamic.0000.nii.gz,dynamic.0000.nii.gz,1,32,Regular,0.25 ] --iterations  500 -u 0 -e 1 --shrinkFactors 4 --smoothingSigmas 2 --transform Affine[ 0.1 ] --metric MI[ dynamic.0000.nii.gz,dynamic.0000.nii.gz,1,32,Regular,0.25 ] --iterations 500 -u 0 -e 1 --shrinkFactors 4 --smoothingSigmas 2 --transform SyN[ 0.3,3,0 ] --metric CC[ dynamic.0000.nii.gz,dynamic.0000.nii.gz,1,4 ] --iterations 70 -u 0 -e 1 --shrinkFactors 4 --smoothingSigmas 2 > antsmotioncorr.log 2>&1;
	cd $(@D); /opt/apps/ANTS/dev/install/bin/antsMotionCorr -v 1 -d 3 --use-histogram-matching 1 -o [motcorr,motcorr.nii.gz] --transform Rigid[ 0.1 ] --metric MI[ dynamic.0000.nii.gz,dynamicc4d.nii.gz,1,32,Regular,0.25 ] --iterations  500x250x100 -u 0 -e 1 --shrinkFactors 4x2x1 --smoothingSigmas 2x1x0 --transform Affine[ 0.1 ] --metric MI[ dynamic.0000.nii.gz,dynamicc4d.nii.gz,1,32,Regular,0.25 ] --iterations 500x250x100 -u 0 -e 1 --shrinkFactors 4x2x1 --smoothingSigmas 2x1x0 --transform SyN[ 0.3,3,0 ] --metric CC[ dynamic.0000.nii.gz,dynamicc4d.nii.gz,1,4 ] --iterations 70x50x20 -u 0 -e 1 --shrinkFactors 4x2x1 --smoothingSigmas 2x1x0 > antsmotioncorr.log 2>&1;
...
Processed/0004/setup:
	...
	c3d -verbose -mcs  '/mnt/FUS4/data2/ethompson/CT_Perfusion/ZPAF19L017_Processed/10.31.2019_Processed/DynMulti4D  1.5  B20f 34 - as a 34 frames MultiVolume by ImagePositionPatientAcquisitionTime.nhdr' -foreach -region 140x130x50vox 128x128x32 -endfor -oo $(@D)/dynamic.%04d.nii.gz 
	c4d -verbose -mcs $(@D)/dynamic.????.nii.gz -tile w  -o   Processed/0004/dynamicc4d.nii.gz
