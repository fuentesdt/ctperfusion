...
Processed/%/dynamicantsreg.nhdr: 
	...
	cd $(@D); /opt/apps/ANTS/dev/install/bin/antsMotionCorr -v 1 -d 3 -o [motcorr,motcorr.nii.gz] -m gc[ dynamic.0000.nii.gz , dynamic.nrrd , 1 , 1 , Random, 0.05  ] -t Affine[ 0.005 ] -i 20 -e 1 -s 0 -f 1  -m CC[  dynamic.0000.nii.gz , dynamic.nrrd , 1 , 2 ] -t GaussianDisplacementField[0.15,3,0.5] -i 20 -e 1 -s 0 -f 1 -n 34  > origantsmotioncorr.log 2>&1;
	cd $(@D); /opt/apps/ANTS/dev/install/bin/antsMotionCorr -v 1 -d 3 --use-histogram-matching 0 -o [motcorr,motcorr.nii.gz] --transform Rigid[ 0.1 ] --metric MI[ dynamic.0000.nii.gz,dynamic.nrrd,1,32,Regular,0.25 ] --iterations  1000x500x250x100 --shrinkFactors 8x4x2x1 --smoothingSigmas 3x2x1x0 --transform Affine[ 0.1 ] --metric MI[ dynamic.0000.nii.gz,dynamic.nrrd,1,32,Regular,0.25 ] --iterations 1000x500x250x100  --shrinkFactors 8x4x2x1 --smoothingSigmas 3x2x1x0 --transform SyN[ 0.1,3,0 ] --metric CC[ dynamic.0000.nii.gz,dynamic.nrrd,1,4 ] --iterations 100x70x50x20 --shrinkFactors 8x4x2x1 --smoothingSigmas 3x2x1x0 -n  34 > antsmotioncorr.log 2>&1;
...
Processed/0004/setup:
	...
	c3d -verbose -mcs '/mnt/FUS4/data2/ethompson/CT_Perfusion/ZPAF19L017_Processed/10.31.2019_Processed/DynMulti4D  1.5  B20f 34 - as a 34 frames MultiVolume by ImagePositionPatientAcquisitionTime.nhdr' -foreach -region 140x130x50vox 256x256x64 -endfor -omc  $(@D)/dynamic.nhdr -oo $(@D)/dynamic.%04d.nii.gz 
